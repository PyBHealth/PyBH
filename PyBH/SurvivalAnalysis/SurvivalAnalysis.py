...
class SurvivalAnalysis:
    ...
    def __init__(
        ...
        self.is_lifelines = (
            "lifelines" in model_module
            or hasattr(model, "print_summary")
            or (hasattr(model, "fit") and hasattr(model, "predict"))
        )
    ...
    def predict_survival_function(self, times, **kwargs):
        ...
        if self.is_bayesian:
            ...
            # Fallback for lifelines if needed, though they usually have
            # predict_survival_function too
            if hasattr(self.model, "predict_survival_function"):
                return self.model.predict_survival_function(times, **kwargs)
            else:
                raise NotImplementedError(
                    "This lifelines model might not support predict_survival_function "
                    "directly."
                )
        else:
